...
public class BST
{
  ...
  public TreeNode remove(int target) {
    ...
    //Traverse to find the target, w/ a piggyback one level behind
    while (ant.getValue() != target) {
        if (ant.getValue() > target) {
            piggyback = ant;
            ant = ant.getLeft();
        } else {
            piggyback = ant;
    ...
    if (ant.getLeft() != null && ant.getRight() != null) {
        //Search for the largest number in the left subtree
        piggyback = ant.getLeft();
        while (piggyback.getRight() != null) {
            piggyback = piggyback.getRight();
        }
    ...
    if (isLeaf(ant)) {
        //If the target is to the left of its parent, have the parent disown its left child
        if (piggyback.getValue() > ant.getValue()) {
        ...
        //Special case: The target node is the only node in the tree
        else _root = null;
    ...
    //Special case: The target is the root, and it has one child
    else {
        //If the child is to the left of the root, make that child the new root
        if (_root.getLeft() != null) {
            _root = _root.getLeft();
        } else
        //If the child is to the right of the root, make that child the new root
